...
def hkdf(message, length):
    salt = b'\x1bq\xea\x112G\x8f \xceumf\xf3\x84\x0f\x96'
    info = b'\x97i?\x1eF\x86\xc8J\xcf\x8d[\xa8\xa4\xd0ef'
    prk = hmac.new(salt, message, hashlib.sha256).digest()
    t = ''
    okm = ''
    for i in range(1, (length // 32) + 1):
        t = hmac.new(prk, t.encode() + info + bytes([i]), hashlib.sha256).hexdigest()
        okm += t
        if length % 32 != 0:
            t = hmac.new(prk, t.encode() + info + bytes([i + 1]), hashlib.sha256).hexdigest()
            okm += t[:length % 32]
    return okm
...
def encrypt():
    d1 = 26653832531469508653818721606246303010725980801560444060629289383328598658747
    d2 = 96283244193505412093934578258513529410049207159365714209006690252322315115439
    d = inverse((d1 * d2), n) - 1
    Px = d * Gx % n
    Py = d * Gy % n
    ...
    x2 = k * Px % n
    y2 = k * Py % n
    M = secrets.token_hex(32)
    t = hkdf((str(x2)+str(y2)).encode(), 32)
    C2 = xor_hex_strings(M, t)
    txt = str(x2) + '@' + M + '@' + str(y2)
    Hash = hashlib.sha256()
...
while input('输入y开始 : ') == 'y':
    ...
    while True:
        print('发送 T1 :', T1)
        c = rsa_encrypt(str(T1).encode(), public_key_p2)
        s.sendall(c)
        data = s.recv(4096)
    ...
    t = hkdf((str(x2)+str(y2)).encode(), 32)
    M_ = xor_hex_strings(C2, t)

    text = str(x2) + '@' + M_ + '@' + str(y2)
    Hash = hashlib.sha256()
