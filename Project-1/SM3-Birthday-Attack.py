...
def birthday_attack():
    ...
    for i in range(len(attack_size)):
        ...
        while True:
            ...
            # 计算对应的hash值
            same_value = SM3.Hash_sm3(message)[:attack_size[i]]
            # 如果该hash值存在且对应消息值不同，那么碰撞找到，结束
            if same_value in hash_table and message != hash_table[same_value]:
                end_time = time.time()
                use_time = end_time - stat_time
                print("Message 1:", message, "Hash Value :", SM3.Hash_sm3(message))
                print("Message 2:", hash_table[same_value], "Hash Value :", SM3.Hash_sm3(hash_table[same_value]))
